...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create a Second Brain decision from an agent session.")
    parser.add_argument("--memory-id", help="Memory id that should own the decision.")
    ...
    parser.add_argument("--yes", action="store_true", help="Actually create the decision. Without this, only preview.")
    ...
    if not args.yes:
        print_json({"preview": payload, "message": "Add --yes to create this decision."})
        return
    print_json(client.post("/decisions", payload))
